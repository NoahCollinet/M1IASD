...
fichiertest = "testGraph.col"

now = Popen(["./parserExe"+ " " +fichiertest], stdout=subprocess.PIPE, shell=True, universal_newlines=True)

Sommet = now.communicate()[0]

nbSousAnneaux = int(Sommet)
portServeur = input("Quel port pour le serveur ? ")

lineUp("Lancement du serveur")
os.system("bin/serveur " + portServeur + " " + str(nbSousAnneaux) + " "  + fichiertest+ "&")
