...
public:
    ...
    Transform(Mat3 i_transformation = Mat3::Identity(), Vec3 i_translation = Vec3(1., 0., 0.))
...
Vec3 project(Vec3 const &input_point, Vec3 const &i_origin, Vec3 const &i_axis) {
    Vec3 distance = input_point - i_origin ;
    distance.normalize();
    Vec3 res1 = Vec3::dot(distance, i_axis) * i_axis;
    float res2 = i_axis.squareLength();
    //Question 2.3: TODO, projeter input_point sur l'axe
    Vec3 result = i_origin + res1 / res2 ;
    return result;
}
