...
public:
  ...
  static Vec3 cross(Vec3 const &a, Vec3 const &b) {
    Vec3 res = Vec3(a[1] * b[2] - a[2] * b[1], -(a[0] * b[2] - a[2] * b[0]),
                    a[0] * b[1] - a[1] * b[0]);
    return res;
  }
...
public:
  ...
  Vec3 operator*(const Vec3 &p) {
    ...
    Vec3 res =
        Vec3((*this)(0, 0) * p[0] + (*this)(0, 1) * p[0] + (*this)(0, 2) * p[0],
             (*this)(1, 0) * p[1] + (*this)(1, 1) * p[1] + (*this)(1, 2) * p[1],
             (*this)(2, 0) * p[2] + (*this)(2, 1) * p[2] +
                 (*this)(2, 2) *
                     p[2]); // Faire le calcul ici et assigner le resultat à res
    return res;
  }

  Mat3 operator*(const Mat3 &m2) { // calcul du produit matriciel m1.m2
    // TODO: Fonction à completer
    // Pour acceder a un element de la premiere matrice (*this)(i,j) et de
    // la deuxième m2(k,l)
    Mat3 res = Mat3(
        (*this)(0, 0) * m2(0, 0) + (*this)(0, 1) * m2(1, 0) +
            (*this)(0, 2) * m2(2, 0),
        ...
        (*this)(2, 0) * m2(0, 2) + (*this)(2, 1) * m2(1, 2) +
            (*this)(2, 2) *
                m2(2,
                   2)); // Faire le calcul ici et assigner le resultat à res
    return res;
  }
