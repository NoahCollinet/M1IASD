...
public:
  ...
  Transform(Mat3 i_transformation = Mat3::Identity(),
            Vec3 i_translation = Vec3(0., 0., 0.))
...
void drawSmoothTriangleMesh(Mesh const &i_mesh) {
  ...
  for (unsigned int tIt = 0; tIt < i_mesh.triangles.size(); ++tIt) {
    ...
    // i_mesh.triangles[tIt][i] --> indice du sommet vi du triangle dans la
    // liste de sommet pi --> position du sommet vi du triangle ni --> normal du
    // sommet vi du triangle pour un affichage lisse
...
void drawTriangleMesh(Mesh const &i_mesh) {
  ...
  for (unsigned int tIt = 0; tIt < i_mesh.triangles.size(); ++tIt) {
    ...
    // Vertices --> liste indexée de sommets
    // i_mesh.triangles[tIt][i] --> indice du sommet vi du triangle dans la
    // liste de sommet pi --> position du sommet vi du triangle
    Vec3 p0 = i_mesh.vertices[i_mesh.triangles[tIt][0]];
    Vec3 p1 = i_mesh.vertices[i_mesh.triangles[tIt][1]];
    Vec3 p2 = i_mesh.vertices[i_mesh.triangles[tIt][2]];
...
void drawMesh(Mesh const &i_mesh) {
  ...
  } else if (i_mesh.triangle_normals.size() > 0)
    drawTriangleMesh(i_mesh); // Display with triangle normals
  else
    drawFlatMesh(i_mesh);
}

void drawVectorField(std::vector<Vec3> const &i_positions,
...
int main(int argc, char **argv) {
  ...
  openOFF("../../data/avion_n.off", mesh.vertices, mesh.normals,
          mesh.triangles, mesh.triangle_normals);

  // Ajout des sommets et triangles au maillage gris (i.e. le maillage
  // transformé)
  ...
  // Changer la matrice pour appliquer une transformation aux sommets du
  // maillage gris
  Mat3 rotation =
      Mat3(cos(90 / 3.14159 * 180), -sin(90 /  3.14159 * 180), 0.,
           sin(90 / 3.14159 * 180), cos(90 / 3.14159 * 180), 0., 0., 0., 1.);

  Mat3 rotation1 =
      Mat3(cos(90/ 3.14159 * 180), 0., sin(90/3.14159 * 180),
           0., 1., 0., -sin(90/3.14159 * 180), 0., cos(90/3.14159 * 180));

  Mat3 rotation2 =
      Mat3(1., 0., 0., 0., cos (90/3.14159 *180),
      -sin(90/3.14159 *180), 0., sin(90/3.14159*180), cos(90/3.14159 * 180));

  Mat3 scale = Mat3(1, 0., 0.,
                    0., 1, 0.,
                    0., 0., 1);

  Mat3 transformation = rotation1 * scale ;
  // Mat3(2., 0., 0., 0., 2., 0., 0., 0., 2.);
  // Add a translation
  Vec3 translation = Vec3(0.75, 0., 0.2);

  for (unsigned int i = 0; i < transformed_mesh.vertices.size(); i++) {
    transformed_mesh.vertices[i] =
        transformation * transformed_mesh.vertices[i] + translation;
  }

  // Utilisation de la matrice de transformation pour transformer les normales
  // du maillage Que constatez-vous si vous appliquez une mise à l'echelle
  // non-uniforme ?
  for (unsigned int i = 0; i < transformed_mesh.normals.size(); i++) {
    transformed_mesh.triangle_normals[i] =
        transformation * transformed_mesh.triangle_normals[i];
